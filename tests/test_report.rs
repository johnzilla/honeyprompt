...
fn insert_event(
    ...
    let extra = format!(r#"{{"classification":"{}","headers":{{}}}}"#, classification);
    conn.execute(
...
fn test_report_with_events() {
    ...
    insert_event(&conn, "n001", 1, "p1", "html_comment", "sess-a", "1.1.1.1", "AgentX/1.0", "Unknown", None);
    insert_event(&conn, "n002", 2, "p2", "meta_tag", "sess-b", "2.2.2.2", "AgentY/1.0", "Unknown", None);
    insert_event(&conn, "n003", 1, "p1", "html_comment", "sess-c", "3.3.3.3", "Googlebot/2.1", "KnownCrawler:Google", None);

    let md = report::generate_report(&conn).expect("generate_report must succeed");

    assert!(md.contains("## Executive Summary"), "must have Executive Summary");
    assert!(md.contains("## Evidence Table"), "must have Evidence Table");
    assert!(md.contains("## Known Crawler Sessions"), "must have Known Crawler Sessions section");

    // Detection sessions = 2 (excluding KnownCrawler)
    assert!(md.contains("| Total Detection Sessions | 2 |"), "detection_sessions must be 2");
}
...
fn test_report_separates_crawlers() {
    let (_tmp, conn) = temp_conn();
    insert_event(&conn, "n010", 1, "p1", "html_comment", "sess-agent", "10.0.0.1", "AgentBot/1.0", "Unknown", None);
    insert_event(&conn, "n011", 1, "p1", "html_comment", "sess-crawler", "10.0.0.2", "Googlebot/2.1", "KnownCrawler:Google", None);

    let md = report::generate_report(&conn).expect("generate_report must succeed");

    // sess-agent should appear in evidence table (before Known Crawler Sessions section)
    let evidence_section_start = md.find("## Evidence Table").expect("must have Evidence Table");
    let crawler_section_start = md.find("## Known Crawler Sessions").expect("must have Known Crawler Sessions");
    ...
    assert!(!crawler_section.contains("sess-agent"), "agent must NOT appear in crawler section");
    // sess-crawler must NOT be in evidence table
    assert!(!evidence_section.contains("sess-craw"), "crawler must NOT appear in evidence table");
}
...
fn test_report_session_based_counting() {
    ...
    insert_event(&conn, "n020", 1, "p1", "html_comment", "sess-same", "1.1.1.1", "Agent/1.0", "Unknown", None);
    insert_event(&conn, "n021", 1, "p2", "meta_tag", "sess-same", "1.1.1.1", "Agent/1.0", "Unknown", None);

    let md = report::generate_report(&conn).expect("generate_report must succeed");

    // Both events share session_id + tier 1 — should count as 1 unique (session, tier) pair
    assert!(md.contains("| Total Detection Sessions | 1 |"), "two events with same session+tier must count as 1 detection");
}
...
fn test_report_timestamp_formatting() {
    ...
    insert_event(&conn, "n030", 1, "p1", "html_comment", "sess-ts", "5.5.5.5", "Agent/1.0", "Unknown", Some(1_700_000_000));

    let md = report::generate_report(&conn).expect("generate_report must succeed");

    // Must contain a human-readable date, not a raw epoch string like "1700000000"
    assert!(!md.contains("| 1700000000"), "report must not contain raw epoch seconds");
    // Must contain 2023 (the year for epoch 1700000000)
    assert!(md.contains("2023"), "report must contain human-readable year 2023");
}
