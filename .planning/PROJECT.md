...
| v5.0 3-phase split (Backend → Surfacing → Validation/Docs) | Smallest blast radius with demoable boundaries at each phase | ✓ Good — shipped same-day |
| `verification_seed` derived from nonce | `u32::from_str_radix(&nonce[0..8], 16)` avoids a schema column; replay semantics stay uniform across tiers | ✓ Good — no new storage surface |
| T5 formula shape `((seed+a)*b) %mod` uniform | All T5 templates share arithmetic shape; only constants vary. Instructions vary in wording, not math | ✓ Good — one verifier fits all |
| T4 sanitization regex `^[a-z0-9_,.\\-]{1,256}$` after base64 decode | Agent-chosen safe menu; lowercase normalization + whitespace strip before regex; malformed → 204 with nothing stored | ✓ Good — preserves safety guarantee |
| Self-identifying T5 seed JSON-LD (carries both `verification_seed` and `nonce`) | Multiple T5 blocks on one page disambiguate; server uses URL-path nonce as authoritative | ✓ Good — ambiguity-free |
| Always-show chrome policy (TUI + report render zero-count T4/T5 rows) | Defenders see the full 5-tier model regardless of DB contents; v4.0 databases still render sensibly | ✓ Good — communicates capability |
| Fixed `[bool; 5]`/`[u32; 5]` scorecard shape over Vec | Matches prior pattern, compile-time bounds; v5.0 is terminal for tier expansion | ✓ Good — minimal churn |
| Symmetric scorecard verdict extension (5 tiers, same 3 verdict values) | Preserves existing `FULLY_COMPLIANT`/`PARTIALLY_COMPLIANT`/`NO_COMPLIANCE` contract for CI consumers | ✓ Good — zero breakage |
| Backward-compat concentrated in Phase 13 | Risk lives where schema + routes change; Phases 14/15 are purely additive on top | ✓ Good — clean layering |
